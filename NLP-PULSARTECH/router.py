...
# --- LÜTFEN GÜNCELLEYİN ---
INTENT_MODEL_PATH = "C:/Users/NETCC/Documents/Yeni klasör/Intent"
NER_MODEL_PATH = "C:/Users/NETCC/Documents/Yeni klasör/Ner_Search"
...
async def lifespan(app: FastAPI):
    ...
    except Exception as e: print(f"[ROUTER] [HATA] Modeller yüklenemedi: {e}")
    print("[ROUTER] --- Model Yükleme Tamamlandı ---")
    yield
    print("[ROUTER] [INFO] Servis durduruldu.")

class QueryRequest(BaseModel): user_query: str
...
app = FastAPI(title="Conditional Local Router Service", lifespan=lifespan)
...
async def call_gemini_for_queries(user_query: str):
    try:
        ...
        prompt = f'Kullanıcı Sorusu: "{user_query}"\nBu soruya en uygun 1-3 Google arama sorgusunu JSON listesi olarak ver. Örnek: ["sorgu 1"]'
...
async def create_execution_plan(request: QueryRequest):
    if not intent_pipeline: raise HTTPException(status_code=500, detail="Niyet modeli yüklenemedi.")
    ...
    if action_type == "RAG_SEARCH":
        if ner_pipeline:
            entities_raw = ner_pipeline(request.user_query)
            extracted_entities = [dict(e, score=float(e['score'])) for e in entities_raw]
            if extracted_entities: print(f"[ROUTER] Varlıklar: {json.dumps(extracted_entities, indent=2, ensure_ascii=False)}")
    ...
    return ExecutionPlan(action_type=action_type, detailed_intent=detected_intent, extracted_entities=extracted_entities, optimized_queries=optimized_queries)